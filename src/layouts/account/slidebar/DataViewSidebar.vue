...
<template>
  <vs-sidebar click-close position-right parent="body" default-index="1" color="primary"
    ...
    <VuePerfectScrollbar style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);" v-if="dataType != 'addMoney'"
      class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- EMAIL -->
        <vs-input label="Hộp thư" v-model="dataEmail" class="mt-5 w-full" type="email" name="item-email"
        ...
        <!-- NICKNAME -->
        <vs-input label="Biệt danh" v-model="dataNick" class="mt-5 w-full" name="item-nick"
        ...
        <span class="text-danger text-sm" v-show="errors.has('item-first_name')">{{ errors.first('item-first_name')
        }}</span>

        <vs-input label="Tên" v-model="dataLastName" class="mt-5 w-full" name="item-last_name" v-validate="'required'" />
        <span class="text-danger text-sm" v-show="errors.has('item-last_name')">{{ errors.first('item-last_name')
        }}</span>

        <!-- PASSWORD -->
        <vs-input ref="password" type="password" data-vv-validate-on="blur" v-validate="'min:6|max:16'" name="password"
          label="Mật khẩu" v-model="dataPassword" class="mt-5 w-full" />
        ...
        <template v-else>
          <vs-select v-model="dataPermission" label="Quyền" class="mt-5 w-full" name="item-category"
            v-validate="'required'">
            <vs-select-item :key="item.value" :value="item.value" :text="item.text" v-for="item in category_choices" />
          </vs-select>
          <span class="text-danger text-sm" v-show="errors.has('item-category')">{{ errors.first('item-category')
          }}</span>

          <vs-select v-if="dataPermission" v-model="isLevelVIP" label="Cấp" class="mt-5 w-full" name="item-category"
            v-validate="'required'">
    ...
    <VuePerfectScrollbar v-else style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);"
      class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- WALLET -->
        <vs-input label="Ví VNĐ" icon-pack="feather" icon="icon-dollar-sign" class="mt-5 w-full" v-model="addDataMoneyVN"
          v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }" name="item-wallet_vn" />
        <span class="text-danger text-sm" v-show="errors.has('item-wallet_vn')">{{ errors.first('item-wallet_vn')
        }}</span>

        <vs-input label="Ví BTC" icon-pack="feather" icon="icon-dollar-sign" class="mt-5 w-full" v-model="addDataMoneyBTC"
          v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }" />

        <vs-input label="Ví ETH" icon-pack="feather" icon="icon-dollar-sign" class="mt-5 w-full" v-model="addDataMoneyETH"
          v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }" />

        <vs-input label="Ví USDT" icon-pack="feather" icon="icon-dollar-sign" class="mt-5 w-full"
          v-model="addDataMoneyUSDT" v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }" />
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        let { email, id, nick_name, first_name, last_name, 
          // money_vn, money_btc, money_eth, money_usdt, 
          profile_image, type, vip_user, super_user, level_vip } = JSON.parse(JSON.stringify(this.data))
