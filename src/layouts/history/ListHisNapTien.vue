            ...
            <tbody>
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td class="whitespace-no-wrap text-center">
                      <vx-tooltip v-if="tr.status === 0" style="float: left" color="warning" text="Duyệt">
                          <vs-button color="dark" type="line" icon-pack="feather" icon="icon-check" @click.stop="acceptDeposit(tr.id, 1)"></vs-button>
                      </vx-tooltip>
                       <vx-tooltip v-if="tr.status === 0" style="float: left" color="warning" text="Hủy">
                          <vs-button color="dark" type="line" icon-pack="feather" icon="icon-x" @click.stop="acceptDeposit(tr.id, 2)"></vs-button>
                      </vx-tooltip>
...
export default {
  ...
  methods: {
    ...
    acceptDeposit(id, status) {
      ...
      AuthenticationService.acceptDepositById(obj)
        .then((resp) => {
          if(resp.data.success){
            this.reloadList()
              // Vue.delete(this.productsFake, index)
